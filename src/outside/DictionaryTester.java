...
public class DictionaryTester {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Now performing " + (DICTIONARY_SIZE * 10) + " searches.");
        ...
        for (int i = 0; i < DICTIONARY_SIZE * 10; i++) {
            ...
            if (val == null)
                throw new RuntimeException("ERROR, key " + testArray[index] + " was not found");
            if (val.intValue() != index)
                throw new RuntimeException("ERROR, wrong value returned, expected " + index + " but got " + val);
        }
        ...
        for (int i = 0; i < DICTIONARY_SIZE / 10; i++) {
            ...
            if (key == null) {
                ...
                System.out.println((Integer) dictionary.getValue(testArray[index]) + "\n");
                throw new RuntimeException("ERROR, key " + testArray[index] + " was not found");
            }
            if (!testArray[index].equals(key))
                throw new RuntimeException(
                        "ERROR, wrong key returned, expected " + testArray[index] + " but got " + key);
        }
        stop = System.currentTimeMillis();
        System.out.println("TIME: " + (stop - start) + "\n");
                    ...
                    for (int l = 0; l < 26; l += (int) 2 * Math.random() + 1)
                        for (int m = 0; m < 26; m += (int) 2 * Math.random() + 1)
                            for (int n = 0; n < 26; n++) {
                                if (where >= size)
                                    break;
                                str[where++] = "" + ((char) (b[0] + n)) + ((char) (b[1] + m)) + ((char) (b[2] + l))
                                        + ((char) (b[3] + k)) + ((char) (b[4] + j)) + ((char) (b[5] + i));
                            }
